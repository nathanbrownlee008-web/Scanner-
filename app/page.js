...
function joinedCount(result){
  return result.homeForCount + result.homeAgainstCount + result.awayForCount + result.awayAgainstCount;
}
...
export default function App(){
  ...
  const currentVsAverageMismatch = market !== avgCalc.market;
  const typicalRangeText = `${avgCfg.typicalMin} to ${avgCfg.typicalMax} per team`;
  const averageModeSummary = avgCalc.mode === "split"
    ? "Recommended: use the home team's last 5 HOME games and the away team's last 5 AWAY games only."
    : "Quick mixed mode: faster, but it mixes home and away games together so accuracy drops.";
        ...
        <section className="avgGrid">
          <div className="card">
            ...
            <div className="infoBox">
              <strong>Current market:</strong> {avgCfg.emoji} {avgCfg.label}<br/>
              <span className="small">Typical per-team range: {typicalRangeText}</span><br/>
              <span className="small">{averageModeSummary}</span>
            </div>

            {currentVsAverageMismatch && (
              <div className="warn">
                Your Averages market is <strong>{avgCfg.label}</strong> but Scanner is currently on <strong>{cfg.label}</strong>. Tapping <strong>Use averages in scanner</strong> will auto-switch Scanner to {avgCfg.label}.
              </div>
            )}
            ...
            <div className="formGrid">
              ...
              <div className="field">
                ...
                <textarea className="textarea" value={avgCalc.homeForSeries} onChange={(e)=>setAvgCalc(s=>({...s, homeForSeries:e.target.value}))} placeholder="Paste numbers only: 4,5,3,6,4" />
                <div className="helperText">No scores here — just the raw stat numbers from each game.</div>
              </div>
              <div className="field">
                ...
                <textarea className="textarea" value={avgCalc.homeAgainstSeries} onChange={(e)=>setAvgCalc(s=>({...s, homeAgainstSeries:e.target.value}))} placeholder="Paste numbers only: 3,4,4,2,5" />
                <div className="helperText">For split mode this should be home-team numbers from HOME games only.</div>
              </div>
              <div className="field">
                ...
                <textarea className="textarea" value={avgCalc.awayForSeries} onChange={(e)=>setAvgCalc(s=>({...s, awayForSeries:e.target.value}))} placeholder="Paste numbers only: 5,3,4,6,5" />
                <div className="helperText">For split mode this should be away-team numbers from AWAY games only.</div>
              </div>
              <div className="field">
                ...
                <textarea className="textarea" value={avgCalc.awayAgainstSeries} onChange={(e)=>setAvgCalc(s=>({...s, awayAgainstSeries:e.target.value}))} placeholder="Paste numbers only: 4,5,3,4,4" />
                <div className="helperText">Keep the same market all the way through — do not mix goals with corners, SOT, or cards.</div>
              </div>
            </div>
          ...
          <div className="avgBox">
            ...
            <div className="rangeGrid">
              <div className="rangeStat"><div className="k">Selected market guide</div><div className="v">{avgCfg.label}</div><div className="small">Typical range: {typicalRangeText}</div></div>
              <div className="rangeStat"><div className="k">Numbers entered</div><div className="v">{joinedCount(avgResult)} values</div><div className="small">Use at least 4 values in each box if you can.</div></div>
            </div>
      ...
      {activeTab==="scanner" && (
        <section className="mainGrid">
          <div className="card">
            ...
            <p className="sectionSub">{cfg.lineHint} · Use the Averages tab first, then paste the bookmaker line and odds here.</p>
            <div className="formGrid">
